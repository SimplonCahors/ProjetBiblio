...
function captcha_text_login_check($user, $username, $password) {
   global $captcha_array, $error_data_array;
   $captcha_bank_logical_error = __("ERROR", "captcha-bank");
   $err = captcha_login_errors();
   $ip_address = sprintf("%u",ip2long(getIpAddress_for_captcha_bank()));
   if ($err) {
      if ($err == "empty") {
      ...
      esc_attr($captcha_array["case_sensitive"]) == "enable" ? $captcha_challenge_field = trim(esc_attr($_REQUEST["ux_txt_captcha_challenge_field"])) : $captcha_challenge_field = strtolower(trim(esc_attr($_REQUEST["ux_txt_captcha_challenge_field"])));
      esc_attr($captcha_array["case_sensitive"]) == "enable" ? $captcha_code[] = $_SESSION["captcha_code"] : $captcha_code[] = array_map("strtolower", $_SESSION["captcha_code"]);
...
function captcha_errors($errors = NULL) {
   global $wpdb, $captcha_array;
   if (isset($_REQUEST["ux_txt_captcha_challenge_field"])) {
      $captcha_challenge_field = $captcha_array["case_sensitive"] == "enable" ? trim(esc_attr($_REQUEST["ux_txt_captcha_challenge_field"])) : strtolower(trim(esc_attr($_REQUEST["ux_txt_captcha_challenge_field"])));
      ...
      if (strlen($captcha_challenge_field) <= 0) {
         ...
         if (isset($_SESSION["captcha_code"])) {
            esc_attr($captcha_array["case_sensitive"]) == "enable" ? $code[] = $_SESSION["captcha_code"] : $code[] = array_map("strtolower", $_SESSION["captcha_code"]);
            if (!in_array($captcha_challenge_field, $code[0])) {
               $errors = "invalid";
               $captcha_meta_settings["captcha_status"] = 0;
            } else {
